...
class ATRStrategy(BaseStrategy):
    ...
    def _algo(data: pd.DataFrame, para_atr_window: int):
        ...
        atr = ta.atr(high=pd.Series(data.High), low=pd.Series(data.Low), close=pd.Series(data.Close), length=para_atr_window)
        return atr
    ...
    def init(self):
        ...
        self.atr_enter = self.I(ATRStrategy.algo_threshold, self.para_atr_enter, len(self.data.Close), name=f"Buy (ATR {self.para_atr_enter})", plot=plot)
        self.atr_exit = self.I(ATRStrategy.algo_threshold, self.para_atr_exit, len(self.data.Close), name=f"Sell (ATR {self.para_atr_exit})", plot=plot)
        self.atr = self.I(ATRStrategy._algo, self.data, self.para_atr_window, name=f"ATR({self.para_atr_window})", plot=plot)
    ...
    def get_optimizer_parameters() -> dict:
        def constraint(p):
            return p.para_atr_enter > p.para_atr_exit  # Enter threshold should be above exit
