...
class EmaCrossSmaStrategy(EmaCrossCloseStrategy):
    ...
    def init(self):
        super().init()
        self.sma_short = self.I(SmaCrossSmaStrategy.algo, self.data.Close, self.para_sma_short, overlay=True, name=f"SMA({self.para_sma_short})")
        super().setup(
    ...
    def get_optimizer_parameters() -> dict:
        def constraint(p):
            return p.para_sma_short > p.para_ema_short and (p.para_sma_short - p.para_ema_short) > 10
