...
class UltimateEmaCrossCloseStrategy(UltimateStrategy):
    ...
    def init(self):
        ...
        self.ema_short = self.I(
            EmaCrossCloseStrategy.algo, self.data.Close, self.para_ema_short, overlay=True, name=f"EMA({self.para_ema_short})"
        )
        self.close = self.I(lambda x: x.Close, self.data)

        self.setup(
            buy_signal=lambda: crossover(self.close, self.ema_short) and  self.uo > self.uo_upper,
            sell_signal=lambda: cross(self.close, self.ema_short) and self.uo_lower > self.uo,
        )
