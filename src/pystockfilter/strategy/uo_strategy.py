...
class UltimateStrategy(BaseStrategy):
    ...
    def init(self):
        ...
        self.uo_upper = self.I(UltimateStrategy.algo_threshold, self.para_uo_upper, len(self.data), name=f"Buy ({self.para_uo_upper})", overlay=True)
        self.uo_lower = self.I(UltimateStrategy.algo_threshold, self.para_uo_lower, len(self.data), name=f"Sell ({self.para_uo_lower})", overlay=True)
        self.setup(
