...
class StartBatchOptimizer(StartBase):
    ...
    def define_parameter_space(self, parameter_dict: dict):
        ...
        """
        space = []
        for param_name, param_range in filter(lambda x: x[0].startswith("para_"), parameter_dict.items()):
            if isinstance(param_range, range):
                ...
                logger.warning(f"Unsupported parameter range type for {param_name}: {param_range}")

        return space

    def bayesian_optimization(self, idx: int, strategy: BaseStrategy, commission: float, cash: float, history_months: int, n_calls=50):
        # Filter parameters starting with "para_"
        parameter_dict = dict(filter(lambda x: x[0].startswith("para_"), self.parameters[idx].items()))
        constraint = self.parameters[idx].get("constraint", lambda x: True)  # Use provided constraint or default to True
        ...
        def objective(**params: Dict):
           ...
           # Check if the parameter combination satisfies the constraint
            if not constraint(SimpleNamespace(**params)):
                logger.debug(f"Parameter combination {parameter} fails constraint check.")
            ...
            for symbol in self.ticker_symbols:
                try:
                    result = self.run_parameter_combination(strategy, symbol, commission, cash, history_months, parameter)
                    results.append(result)
                except Exception as e:
                    logger.error(f"Error processing parameter set {parameter} for symbol {symbol}: {e}")
        ...
        logger.info(f"Best parameters found for strategy {strategy.__name__}: {best_params} with SQN score: {-res.fun}")
        return best_params
    
    def run_strategy(self, idx: int, strategy: BaseStrategy, commission: float, cash: float, history_months: int):
        start_time = datetime.now()
        best_params = self.bayesian_optimization(idx, strategy, commission, cash, history_months)
        overall_results = BacktestResult("overall", str(strategy.name), best_params, None, None, None, 0.0, 0.0, 0.0)
        for symbol in self.ticker_symbols:
            df = self.get_data(symbol, history_months)
            result = self.run_implementation(strategy, symbol, df, commission, cash, best_params)
    ...
    def run_parameter_combination(self, strategy, symbol, commission, cash, history_months, parameter):
        df = self.get_data(symbol, history_months)
    ...
    def run(self, commission=0.002, cash=10000.0, history_months=6) -> list[BacktestResult]:
        ...
        for idx, strategy in enumerate(self.strategies):
            ...
            logger.info(f"Optimization completed for strategy {strategy.__name__} with SQN: {result.sqn}")
